...
if flag_voxels01==1
    for i=1:size(goodPairsL,2)
       ...
       goodPairsR_tmp=zeros(size(goodPairsL,1),1);
    ...
    for i=1:size(badPairsL,2)
       ...
       badPairsR_tmp=zeros(size(badPairsL,1),1);
       badPairsR_tmp(ids)=1;
...
goodPairsL = goodPairsL(:,sel_tr);
goodPairsR = goodPairsR(:,sel_tr);
badPairsL = badPairsL(:,sel_tr);
badPairsR = badPairsR(:,sel_tr);
